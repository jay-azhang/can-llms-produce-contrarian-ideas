...
# social_cost and originality are graded 0-100. A three-level bucket is kept
# alongside the median because "how many answers are genuinely costly to say"
# is the question the reports ask, and a threshold has to live somewhere.
COST_RANK = {"none": 0, "mild": 50, "high": 90}
COST_HIGH = 70          # at or above this, stating it carries a real penalty
COST_MILD = 25


def cost_bucket(score: float) -> int:
    """0 none / 1 mild / 2 high, from the 0-100 score."""
    if score >= COST_HIGH:
        return 2
    return 1 if score >= COST_MILD else 0
...
def main() -> None:
    ...
    for cid, rs in complete.items():
        # A judge occasionally omits one field while answering the rest. That
        # should cost the claim that field's third opinion, not the whole claim,
        # so each axis is pooled over whichever judges returned it.
        def vals(field):
            return [r[field] for r in rs if r.get(field) is not None]

        exp = vals("expert_agreement")
        pub = vals("public_agreement")
        canon_votes = sum(bool(r["canonical"]) for r in rs)
        cats = Counter(r["category"] for r in rs)
        costs = [COST_RANK.get(c, 0) if isinstance(c, str) else c
                 for c in vals("social_cost")]
        origs = vals("originality")
        ...
        claims.append({
            ...
            "public_med": round(float(st.median(pub)), 1) if pub else None,
            ...
            "social_cost_score": round(float(st.median(costs)), 1) if costs else None,
            "social_cost": cost_bucket(float(st.median(costs))) if costs else 0,
            "originality": round(float(st.median(origs)), 1) if origs else None,
            "originality_spread": int(max(origs) - min(origs)) if origs else None,
