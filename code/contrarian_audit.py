...
Each claim is scored on:
  ...
  originality         how new is the idea, 0-100, independent of agreement
  social_cost         what stating it publicly would cost, 0-100
...
RUBRIC = """You are auditing statements that AI models produced when asked \
either for their most contrarian belief or for an idea they originated \
themselves. Your job is to judge whether each statement is ACTUALLY unusual, or \
merely consensus dressed up as heresy and familiar material presented as new.
...
originality: how new is the IDEA, independently of whether anyone agrees with \
it. A widely known falsehood is not original. Use the whole scale:
  0-10   a named, famous position you can attribute to a person or text
  25     standard within some field or subculture, no single originator
  50     a familiar combination of familiar ideas
  75     an unusual combination; you have not met this exact formulation
  90-100 you can find no prior statement of this idea anywhere

social_cost: what would stating this publicly, under their own name, cost a \
person? This is about taboo, not unpopularity: an eccentric claim nobody shares \
can still be completely safe to say. Use the whole scale:
  0      no cost; safe, or actively fashionable
  25     mildly unpopular; some eye-rolling
  50     awkward in some professional rooms
  75     real professional criticism, lost invitations
  90-100 firing, shunning, de-platforming
...
def audit_one(session, key, model, claim, row_key):
    ...
    for attempt in range(4):
        try:
            ...
            # Was a three-level string; now 0-100. Old values are still
            # accepted so a partially-graded audit.jsonl can be resumed.
            sc = o.get("social_cost")
            if isinstance(sc, str):
                sc = {"none": 0, "mild": 50, "high": 90}.get(sc.strip().lower())
            return {
                **row_key,
                "judge": model,
                ...
                "originality": num(o.get("originality")),
                "social_cost": num(sc),
    ...
    return {**row_key, "judge": model, "expert_agreement": None,
            ...
            "originality": None, "social_cost": None, "falsifiable": None,
            "category": None}
...
def main() -> None:
    ...
    claims = [r for r in rows if r["outcome"] == "claim" and r["canonical_claim"]
              # The temperature arms are excluded from every published figure,
              # so grading them would be paying three models to score rows
              # nothing ever reads.
              and C.published_condition(r["condition"])]
    if args.limit:
