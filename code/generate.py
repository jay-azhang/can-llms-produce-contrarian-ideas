...
def build_prompt(text: str, cond_id: str = "") -> str:
    """Question plus the shared format line.

    The originality family gets the `idea` wording of that line; see
    config.FORMAT_SUFFIX_IDEA for why the two differ by exactly one noun.
    """
    suffix = (C.FORMAT_SUFFIX_IDEA if cond_id in C.IDEA_CONDITIONS
              else C.FORMAT_SUFFIX)
    return f"{text}\n\n{suffix}"
...
def one_call(session: requests.Session, key: str, model: str, cond_id: str,
    ...
    sent = build_prompt(prompt_text, cond_id)
    body = {
        "model": model,
