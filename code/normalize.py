...
Some answers open by disclaiming provenance ("I can't guarantee this is absent \
from my training data") and then commit to a specific idea. Those are CLAIMS, \
not hedges or refusals: extract the idea and discard the disclaimer. Only code \
such an answer as a refusal if no idea follows the disclaimer.

Where the answer is a proposal rather than a belief ("cities should do X"), \
treat the proposal as the claim.
...
def judge_one(session, key, model, text, row_key, max_tokens=4000) -> dict:
    body = {
        "model": model,
        "messages": [
        ...
        # 1500 still lost 92 of 1,521 rows, all of them recovered at 4000. The
        # cap only buys headroom; it does not change what a successful call costs.
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as pool:
        ...
        for r in rows:
            rk = {"id": row_id(r), "model": r["model"],
                  ...
                  "raw_text": r["text"]}
            futs[pool.submit(judge_one, session, key, model, r["text"], rk,
                             max_tokens)] = rk
...
def main() -> None:
    ...
    ap.add_argument("--retry-errors", action="store_true",
                    help="re-judge only the rows coded judge_error, with a "
                         "bigger token budget, and merge them back in")
    args = ap.parse_args()
    ...
    if args.retry_errors:
        # The judge truncates its own JSON on long answers, and at temperature 0
        # an identical retry truncates identically — so the only fix is more room.
        have = {json.loads(l)["id"]: json.loads(l) for l in open(C.NORMALIZED)}
        bad = [r for r in rows if have.get(row_id(r), {}).get("outcome") == "judge_error"]
        if not bad:
            print("no judge_error rows to retry")
            return
        print(f"retrying {len(bad)} judge_error rows at max_tokens=4000")
        fixed = run_judge(bad, C.JUDGE_MODEL, key, args.concurrency,
                          desc="retry ", max_tokens=4000)
        recovered = sum(1 for r in fixed if r["outcome"] != "judge_error")
        have.update({r["id"]: r for r in fixed})
        with open(C.NORMALIZED, "w") as f:
            for r in have.values():
                f.write(json.dumps(r) + "\n")
        print(f"recovered {recovered} of {len(bad)} -> {C.NORMALIZED}")
        return
